...
const input = `position=<-41150,  41504> velocity=< 4, -4>
...
for (let i = 0; i < input.length; i++) {
    ...
    if ((input.charAt(i) == '-' || Number.isInteger(parseInt(input.charAt(i)))) &&
        input.charAt(bufor) != '-' && isNaN(input.charAt(bufor))) {
...
for (let i = 0; i < graph.length; i++) {
    ...
    if (!was) {
        graph = graph.slice(0, i).concat(graph.slice(parseInt(i) + 1, graph.length));
        i--;
    }
...
for (let i = 0; i < graph[0].length; i++) {
    ...
    if (!was && !was_l) {
        ...
        for (let j = 0; j < graph.length; j++) {
            graph[j] = graph[j].slice(0, i).concat(graph[j].slice(i + 1, graph[j].length));
        }
        i--;
